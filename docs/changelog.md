...
- **Operations only touch image and video files**: `rename parent`, `move` (with structure) and `sort alpha` previously processed every file in the folder, including documents, archives and sidecars; they now handle images only, plus videos with the new `--include-videos` / `includeVideos` option (also added to the web UI). A custom `--extensions`/`extensions` list can no longer pull in non-media types (`config.media_only()`), `move` no longer mirrors folders that contain no media or creates the destination in dry-run mode, and the single-file endpoints (`/api/image/rename`, `/api/delete`, `/api/batch-operation`) reject non-media paths. Added the Sony/Leica RAW extensions `.srf`, `.sr2`, `.rwl` to `IMAGE_EXTENSIONS`.
- **Drive-aware worker counts**: scans and file operations now choose how many files to process at once from each drive's type (HDD/SSD/NVMe) and connection (SATA/USB/SD/network) instead of fixed defaults. Examples: an internal SATA HDD scans with 4 workers and copies with 1; a USB SSD scans with up to 6 and copies with 2; renames on NVMe use 16 (previously 4). Moves between drives are sized by the slower side. A short read-only speed test settles ambiguous USB/RAID/network drives, and long scans/copies adjust concurrency while they run (`pixsieve.utils.worker_policy`, `io_probe`, `adaptive`; `disk_type` now detects the bus and network mounts on Windows, macOS and Linux). New `pixsieve-cli storage` command, `--storage-profile`/`--no-io-probe` flags, `-w` on `move`, `move-to-parent`, `metadata randomize-dates` and `pipeline`, `PIXSIEVE_WORKERS`/`PIXSIEVE_STORAGE_OVERRIDE`/`PIXSIEVE_AUTO_WORKERS`/`PIXSIEVE_IO_PROBE` environment variables, and an Auto toggle plus drive badge in the web UI. **Behavior change:** `workers` now defaults to `null` (auto) in the API and `-w` defaults to `auto` in the CLI; explicitly passing the old default (e.g. `workers: 4`) now means exactly 4 instead of silently enabling large-library/HDD scaling. `PIXSIEVE_AUTO_WORKERS=0` restores the previous fixed counts.
...
- **Sort by resolution**: sorts images into `<category>/<orientation>/` folders (created directly inside the target directory, same layout style as alphabetical sort) by resolution tier (tiny → 8k+) and orientation (`pixsieve.operations.sort.sort_by_resolution`, `POST /api/operations/sort/resolution`). Web GUI / API only.
